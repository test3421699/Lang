...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    if (audio) {
      console.log(`Received base64 audio payload (${audioMime}) for transcription...`);
      try {
        ...
        console.log(`Transcribed text: "${resolvedUserText}"`);
      } catch (transError) {
    ...
    // Translate this into Part/Content structured objects for the Gemini model
    ...
    console.log(`Generating conversational dialogue response with system instructions...`);
    ...
    console.error('Core /api/chat error:', error);
    res.status(500).json({ error: error?.message || 'Something went wrong on our server. Try again!' });
  }
});

// Helper to convert raw PCM (signed 16-bit little-endian, mono, 24kHz) to standard playble WAV format
...
app.post('/api/tts', async (req, res) => {
  try {
    ...
    const ai = getAi();
    console.log(`Generating TTS audio pronunciation for: "${text.substring(0, 40)}..." employing voice "${voice}"`);
    ...
    console.log(`TTS raw response came, length: ${base64AudioRaw?.length || 0}`);
    ...
    console.log(`Successfully converted TTS PCM audio to browser-playable WAV. New length: ${wavBase64.length}`);

    res.json({ audio: wavBase64, mimeType: 'audio/wav' });
  } catch (error: any) {
    console.error('Core /api/tts error:', error);
